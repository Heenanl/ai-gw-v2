...
@description('APIM subscription key for API access. Required when authType is ApiKey, ignored for AAD.')
@secure()
param apimSubscriptionKey string = ''

@allowed(['ApiKey', 'AAD', 'ProjectManagedIdentity'])
@description('Authentication type. ApiKey uses subscription key; AAD/ProjectManagedIdentity uses the project managed identity.')
param authType string = 'ApiKey'

@description('Share connection to all project users')
param isSharedToAll bool = false

@description('Token audience for ProjectManagedIdentity auth (e.g., https://cognitiveservices.azure.com or api://<app-id>)')
...
resource apimConnection 'Microsoft.CognitiveServices/accounts/projects/connections@2025-04-01-preview' = {
  ...
  properties: union({
    ...
    credentials: authType == 'ApiKey' ? { key: apimSubscriptionKey } : {}
    metadata: metadata
